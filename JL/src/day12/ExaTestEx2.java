...
public class ExaTestEx2 {
	...
	public static void main(String[] args) {
		...
		list[2] = new ExaRect(10, 10, 20, 30);
		list[3] = new ExaShape(10, 10, 0, 0);
		//예제3)
		//반복문을 이용하여 배열에 저장된 도형들을
		//콘솔에 출력해보세요(print()이용)
		/* NullPointerException : 객체가 없는데 객체의 기능을
		/* 사용하려고 할 때 발생
		 * 객체가 없다 ==> null
		 * */
		for(int i = 0; i < list.length; i++) {
			...
			//도형이 없으면 아무것도 안함
			//도형이 없다가 null인 이유
			// - 배열은 생성이 되면 타입에 맞는 초기값으로
			// 초기화를 한다
			// - 참조변수는 null로 초기화
			// - 도형이 없다는 건 도형이 생성되어 저장된게
			// 아니기 때문에 초기값인 경우이다
			if(list != null) {
				list[i].print();
			}
		...
		for (ExaShape tmp : list) {
			if(tmp != null) {
				...
				if(tmp instanceof ExaEllipse) {
					((ExaEllipse)tmp).testE(); //한 줄 표현(선택해서사용)
					//ExaEllipse e = (ExaEllipse)tmp; // 두 줄 표현
					//e.testE();                      //(선택해서사용)
				}
